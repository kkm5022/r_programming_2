...
score_type <- aggregate(t(marks), list("유형" = type_kr), sum, na.rm = TRUE)
score_type == 0
score_type[score_type == 0] <- NA
score_type_by.student <- t(score_type[, -1])
N <- nrow(literacy)
dimnames(score_type_by.student) <- list(paste("s", 1:N, sep = ""), paste(levels(type_kr), c("(52점)", "(36점)", "(12점)"), sep=""))
```

```{r, scores by types, echo = FALSE, results = 'hide', eval = TRUE}
pander(score_type_by.student * 4)
```
...
rate.by.item.type <- data.frame("유형" = type_kr, "정답률" = rate.by.item)
