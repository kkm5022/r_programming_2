...
위 과정은 다음과 같이 사용자 함수와 `mapply()` 또는 `sapply()`를 이용하여 다소 줄일 수도 있다.
...
MI_order_kr <- apply(MI_score_kr_ok, 1, order, decreasing = TRUE)
pander(head(t(MI_order_kr)))
MI_sort_kr <- matrix(MI_names_kr[MI_order_kr], 
                     ncol = 8, byrow = TRUE, 
                     dimnames = list(MI$ID[MI_ok_id], paste("제", 1:8, "순위", sep = "")))
