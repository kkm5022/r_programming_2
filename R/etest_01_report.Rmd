...
id_new <- setdiff(id0907, id0901) # 0907에서 0901을 뺀사람들 
# 즉 1일에는 없었지만 7일에 새로 들어온 사람만 저장 
...
#match 첫번째 매개변수가 두번째 매개변수의 몇번째에 포함되는지 나타냄
quiz01$submit <- ifelse(is.na(quiz01$time), "미제출", "제출")
#quiz01$submit %>% '==' (is.na(quiz01$time)) %>% ifelse("미제출","제출")
#quiz01$submit <- (is.na(quiz01$time)) %>% ifelse("미제출","제출")
...
#quiz01$new <- quiz01$id %in% id_new %>% ifelse("new", "stay")
...
#케이블은 표같이 점선을 만들어준다/테이블은 표를 만들어준다(케이블없어도 가능)
```
